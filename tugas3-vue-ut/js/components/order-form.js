Vue.component('order-form',{
  ...
  `,
  data(){
    return { loading:true, paket:[], selected:'', qty:1, buyer:'', alamat:'' }
  },
  computed:{
    paketObj(){ return this.paket.find(p=>p.kode===this.selected)
    }
  },
  methods:{
    async load(){
    ...
    doOrder(){
      if(!this.selected||!this.buyer||!this.alamat){
        alert('Isi semua field'); return; }
        alert('Order berhasil (mock): '+this.selected+' x'+this.qty);
        this.qty=1;
        this.buyer='';
        this.alamat='';
        this.selected='';
