// stock-table component (uses local stok array passed as prop and mutates it)
Vue.component('stock-table',{
  ...
  computed:{
    kategoriByUpbjj(){ if(!this.filterUpbjj) return [...new Set(this.kategoriList)].sort(); return [...new Set(this.stok.filter(s=>s.upbjj===this.filterUpbjj).map(s=>s.kategori))].sort(); },
    filtered(){
  ...
  methods:{
    reset(){ this.filterUpbjj=''; this.filterKategori=''; this.filterStatus=''; this.sortBy=''; },
    openEdit(it){ this.formEdit = Object.assign({}, it); this.modalEdit=true; },
    saveEdit(){ const idx = this.stok.findIndex(x=>x.kode===this.formEdit.kode); if(idx>=0) Vue.set(this.stok, idx, Object.assign({}, this.formEdit)); this.modalEdit=false; },
    openTambah(){ this.modalTambah=true; },
    saveTambah(){ if(!this.formTambah.kode||!this.formTambah.judul||!this.formTambah.upbjj){ alert('Kode, Judul, UPBJJ wajib'); return; } if(this.stok.find(x=>x.kode===this.formTambah.kode)){ alert('Kode sudah ada'); return; } this.stok.push(Object.assign({}, this.formTambah)); this.formTambah={kode:'',judul:'',kategori:'',upbjj:'',lokasiRak:'',harga:0,qty:0,safety:0,catatanHTML:''}; this.modalTambah=false; },
    askDelete(it){ if(confirm('Hapus '+it.kode+' ?')){ const idx=this.stok.indexOf(it); if(idx>=0) this.stok.splice(idx,1); } }
  },
  template: `
  <div>
    <div class="filter-row">
      <div><label>UPBJJ</label><br/><select v-model="filterUpbjj"><option value="">Semua</option><option v-for="u in upbjjList" :key="u" :value="u">{{u}}</option></select></div>
      <div v-if="filterUpbjj"><label>Kategori</label><br/><select v-model="filterKategori"><option value="">Semua</option><option v-for="k in kategoriByUpbjj" :key="k" :value="k">{{k}}</option></select></div>
      <div><label>Status</label><br/><select v-model="filterStatus"><option value="">Semua</option><option value="low">Stok &lt; Safety</option><option value="empty">Stok = 0</option></select></div>
      <div><label>Sort</label><br/><select v-model="sortBy"><option value="">Default</option><option value="judul">Judul</option><option value="harga">Harga</option><option value="qty">Stok</option></select></div>
      <div class="button-group"><button class="btn-add" @click="openTambah">+ Tambah</button><button class="btn-reset" @click="reset">Reset</button><a class="btn-back" href="index.html">Back</a></div>
    </div>

    <div class="table-wrapper">
      <table>
        <thead><tr><th>Kode</th><th>Judul</th><th>Kategori</th><th>UPBJJ</th><th>Lokasi</th><th>Harga</th><th>Stok</th><th>Safety</th><th>Status</th><th>Aksi</th></tr></thead>
        <tbody>
          <tr v-for="it in filtered" :key="it.kode">
            ...
            <td><button class="btn-edit" @click="openEdit(it)">Edit</button> <button class="btn-delete" @click="askDelete(it)">Hapus</button></td>
          </tr>
        </tbody>
      </table>
    </div>
